  ...
  return (
    <>
      ...
      <div className="wrap" style={{ paddingTop: "6.5rem", paddingBottom: "1rem" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "baseline", gap: "0.9rem", marginBottom: "2.5rem" }}>
          {quickJumps.map((q, i) => (
            <span key={q.href} style={{ display: "flex", alignItems: "baseline", gap: "0.9rem" }}>
              ...
              {i < quickJumps.length - 1 ? <span style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: "1rem", color: "var(--color-mist)" }}>/</span> : null}
            </span>
          ))}
        </div>
        <div className="hero-personal" style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: "2.5rem", alignItems: "center" }}>
          <div className="hero-portrait" style={{ width: "9.5rem", flexShrink: 0 }}>
            <ImagePlaceholder label="ảnh chân dung · Hồ Thành Tâm" aspectRatio="1/1" style={{ borderRadius: "50%", overflow: "hidden" }} />
          </div>
          <div>
            ...
            <span style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: "0.95rem", color: "var(--color-stone-alt)" }}>— Tâm</span>
          </div>
        </div>
      </div>

      <div style={{ marginTop: "2.5rem", height: "52vh", minHeight: 320, maxHeight: 500 }}>
        <ImagePlaceholder label="ảnh · không gian tĩnh, be-still" aspectRatio="auto" style={{ height: "100%" }} />
      </div>

      <Reveal className="wrap" style={{ marginTop: "3.5rem", marginBottom: "5rem", maxWidth: "56ch" }}>
        <p style={{ fontFamily: "var(--font-sans)", fontSize: "0.95rem", lineHeight: 1.9, color: "var(--color-ink)", textAlign: "justify", margin: "0 0 1.25rem" }}>be-still không phải nơi chữa lành, cũng không phải nơi tu học.</p>
      ...
      <div className="wrap" style={{ marginBottom: "1.2rem" }}>
        <span className="eyebrow">các không gian thực hành</span>
      </div>
      <RevealStagger className="wrap" style={{ marginBottom: "5rem" }}>
        {practices.map((p) => (
          <Link key={p.name} className="prog-row go" href={p.href}>
            <span style={{ display: "block", fontFamily: "var(--font-mono)", fontSize: "0.6rem", letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--color-stone-alt)", marginBottom: "0.6rem" }}>
              {p.tag}
            </span>
            <span style={{ display: "flex", alignItems: "baseline", gap: "0.6rem", flexWrap: "wrap", marginBottom: "0.5rem" }}>
              <span style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: "1.4rem", color: "var(--color-ink)" }}>{p.name}</span>
              <span className="ar" style={{ color: "var(--color-stone-alt)", fontSize: "0.85rem" }}>
                →
              </span>
            </span>
            <span style={{ display: "block", fontFamily: "var(--font-sans)", fontSize: "0.9rem", lineHeight: 1.7, color: "var(--color-stone-alt)", maxWidth: "52ch" }}>{p.desc}</span>
          </Link>
        ))}
      </RevealStagger>

      <div className="wrap" style={{ marginBottom: "1.2rem" }}>
        <span className="eyebrow">người hướng dẫn</span>
      </div>
      <Reveal className="wrap" style={{ marginBottom: "2rem", maxWidth: "52ch" }}>
        <p style={{ fontFamily: "var(--font-sans)", fontSize: "0.95rem", lineHeight: 1.9, color: "var(--color-ink)", textAlign: "justify", margin: "0 0 1.5rem" }}>
          tôi là Hồ Thành Tâm, người sáng lập{" "}
          <Link className="mono-link" href="/" style={{ fontFamily: "var(--font-sans)", color: "var(--color-ink)" }}>
            mộc little farm
          </Link>
          .
        </p>
      ...
      <div className="wrap" style={{ marginBottom: "2rem", maxWidth: "52ch" }}>
        ...
        <p style={{ fontFamily: "var(--font-sans)", fontSize: "0.82rem", lineHeight: 1.7, color: "var(--color-stone-alt)", margin: "0.6rem 0 0" }}>
          tâm quán là blog cá nhân của tôi — nơi tôi ghi lại những điều mình thấy trên đường.
        </p>
      </div>

      <Reveal className="wrap" style={{ marginBottom: "5rem", maxWidth: "48ch", marginLeft: "auto", marginRight: "auto" }}>
        <div style={{ border: "1px solid var(--color-mist)", padding: "2.5rem 2rem", textAlign: "center" }}>
          <p style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: "1.15rem", lineHeight: 1.75, color: "var(--color-ink)", margin: "0 0 0.5rem" }}>
            Nếu bạn thấy mình cần một khoảng lặng để nhìn lại —
            <br />
            tôi ở đây.
          </p>
          <span style={{ display: "block", fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: "0.9rem", color: "var(--color-stone-alt)", marginBottom: "1.5rem" }}>— Tâm</span>
          <div style={{ display: "inline-flex", flexDirection: "column", gap: "0.6rem", textAlign: "left" }}>
            <a href="https://www.facebook.com/profile.php?id=61586840221190" target="_blank" rel="noopener" style={{ fontFamily: "var(--font-sans)", fontSize: "0.9rem", color: "var(--color-ink)", borderBottom: "1px solid var(--color-mist)", paddingBottom: "0.15rem" }}>
              trang facebook cá nhân
            </a>
            <a href="https://www.instagram.com/tamthanh.ho" target="_blank" rel="noopener" style={{ fontFamily: "var(--font-sans)", fontSize: "0.9rem", color: "var(--color-ink)", borderBottom: "1px solid var(--color-mist)", paddingBottom: "0.15rem" }}>
              trang instagram cá nhân
            </a>
          </div>
        </div>
      </Reveal>
