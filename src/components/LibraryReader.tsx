        ...
        {filtered.length ? (
          <div style={{ opacity: fading ? 0 : 1, transform: fading ? "translateY(-8px)" : "translateY(0)", transition: "opacity 0.4s ease, transform 0.4s ease", display: "flex", gap: "2rem" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <h2 style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontWeight: 300, fontSize: "clamp(1.25rem, 2.8vw, 1.6rem)", lineHeight: 1.3, color: "var(--color-ink)", margin: "0 0 1.25rem", textAlign: "center" }}>
                {current.title}
              </h2>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: "3.5rem" }}>
                <span style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: "1.3rem", color: "var(--color-mist)" }}>❖</span>
              </div>
          ...
          <p style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", color: "var(--color-stone)", fontSize: "0.95rem" }}>không tìm thấy ghi chép nào khớp với từ khoá này.</p>
        )}
      </div>
    </div>
  );
